...
function takeHostage()
	...
	if not canTakeHostage then 
		drawNativeNotification("Tu a besoin d'une Arme avec des Balles pour prendre en otage cette personne")
	end
	...
	if not holdingHostageInProgress and canTakeHostage then		
		...
		if closestPlayer ~= -1 and closestPlayer ~= nil then
			...
			drawNativeNotification("Il n'y a personne a braquer")
		end 
	end
...
Citizen.CreateThread(function()
	while true do 
		if holdingHostage then
			...
			DrawText3D(playerCoords.x,playerCoords.y,playerCoords.z,"Appuye sur [G] pour lacher, [H] pour tuer")
