...
RegisterNetEvent('ddx_teleportpads:hasEnteredMarker')
AddEventHandler('ddx_teleportpads:hasEnteredMarker', function(currentPad, padData)
	CurrentAction = 'pad.' .. string.lower(currentPad)
	CurrentActionMsg = padData.Text
	CurrentActionData = { padData = padData }
end)

RegisterNetEvent('ddx_teleportpads:hasExitedMarker')
AddEventHandler('ddx_teleportpads:hasExitedMarker', function()
	...
	while true do
		...
		if isInMarker and not HasAlreadyEnteredMarker or (isInMarker and (LastPad ~= currentPad or LastAction ~= currentAction)) then
			if (LastPad ~= nil and LastAction ~= nil) and (LastPad ~= currentPad or LastAction ~= currentAction) then
				TriggerEvent('ddx_teleportpads:hasExitedMarker', LastPad, LastAction)
			...
			TriggerEvent('ddx_teleportpads:hasEnteredMarker', currentPad, currentPadData)
		end
