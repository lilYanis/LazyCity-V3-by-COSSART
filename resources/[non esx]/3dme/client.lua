...
local txt = "La personne" -- /me text
local swtxt = "* L'individue équipe: Armes de poing *"
local mwtxt = "* L'individue équipe: Armes de mêlée *"
local hwtxt = "* L'individue équipe: Armes d'assault *"



-- 3dtext settings --

local color = { r = 104, g = 45, b = 230, alpha = 255 } -- color of the text 
local font = 0 -- font of the text
local time = 3 -- duration of the display of the text in seconds
local background = {
	...
	while true do
		...
		if playerPed then
			...
			if daw == true then
				...
				if GetVehiclePedIsIn(playerPed, false) == 0 and isWeaponHeavy(weapon) then
					if heavyWeaponOut == false then
						heavyWeaponOut = true
						if tafsw == true then
							local text = hwtxt
							TriggerServerEvent('3dme:shareDisplay', text)
...
Citizen.CreateThread(function()
	while true do
		...
		if playerPed then
			...
			if not isWeaponHeavy(weapon) and heavyWeaponOut == true then
				Wait(100)
				heavyWeaponOut = false
			end
...
function isWeaponHeavy(model)
	for _, heavyWeapon in pairs(heavyweaponslist) do
		if model == GetHashKey(heavyWeapon) then
			return true
		end
	end
