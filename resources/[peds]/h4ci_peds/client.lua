...
Citizen.CreateThread(function()
    while true do
    	...
    	RageUI.IsVisible(RMenu:Get('ped', 'main'), true, true, true, function()
            ...
            RageUI.Button("Danseuse", "Pour se mettre en chinoise", {RightLabel = ""}, true, function(Hovered, Active, Selected)
                if (Selected) then   
                local j1 = PlayerId()
    			...
    			while not HasModelLoaded(p1) do
   				 ...
   				 SetPlayerModel(j1, p1)
   				 SetModelAsNoLongerNeeded(p1)
   				 ESX.ShowNotification('Tu est maintenant une chinoise')
                end      
            end)
            RageUI.Button("Cosmonaute", "Pour se mettre en Lana", {RightLabel = ""}, true, function(Hovered, Active, Selected)
                if (Selected) then   
                local j1 = PlayerId()
    			...
    			while not HasModelLoaded(p1) do
   				 ...
   				 SetPlayerModel(j1, p1)
   				 SetModelAsNoLongerNeeded(p1)
   				 ESX.ShowNotification('Tu est maintenant Lana')
                end      
            end)
             RageUI.Button("Alien", "Pour se mettre en enfant", {RightLabel = ""}, true, function(Hovered, Active, Selected)
                if (Selected) then   
                local j1 = PlayerId()
